...
export default function Page() {
  ...
  return <main className={menu ? 'menu-open' : ''}>
    ...
    <footer><div className="footer-brand"><a href="#top" className="logo" aria-label="JO Painting, LLC — Home"><img src="/logo.png" alt="JO Painting, LLC" /></a><p>Premium exterior cleaning for properties that deserve a fresh start.</p></div><div className="footer-contact"><a href={`tel:${phone}`}><Phone size={15} /> {phone}</a><a href="mailto:info@jopainting.com"><Mail size={15} /> info@jopainting.com</a><span><MapPin size={15} /> Newport County, Rhode Island</span></div><div className="footer-links">{['Home', 'Services', 'Results', 'Reviews', 'Contact'].map(x => <a key={x} href={`#${x === 'Home' ? 'top' : x.toLowerCase()}`}>{x}</a>)}</div><small>© 2026 {brand}. All rights reserved.</small></footer>
